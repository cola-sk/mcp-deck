...
MCP Deck 把这些客户端的配置统一收口到一个 **可编辑的源文件** 上，提供图形化界面来增删改 MCP 服务器，并一键同步到所有支持的客户端。
...
- 一键同步会把源文件中的所有服务器写入各自的目标客户端；没有 MCP Deck 元数据的服务器默认同步给所有支持的客户端。
